#!/usr/bin/env python
"""λ=0.10 retune — test whether a stronger magnitude penalty stabilises std_ratio.

Trains poc_a and tcn with magnitude_penalty_lambda=0.10, then overlays
circular-loss + std_ratio trajectories against both the Run 7 λ=0.01 baseline
and the λ=0 ablation, giving a 3-point curve (0, 0.01, 0.10) for each metric.
...
Run this after (or instead of) run_lam005_retune.py if λ=0.05 doesn't fully
stabilise std_ratio.

After training + plotting + evaluation, runs diagnostic_lam010_retune.py
(multi-step prediction-perturbation trace + std_ratio/circular-loss health
check) on the fresh checkpoints.

Usage:
    python experiments/phic_psi_poc/run_lam010_retune.py
    python experiments/phic_psi_poc/run_lam010_retune.py --compare-only
"""
...
RETUNE_CONFIGS = [
    "config_lam010_retune.yaml",       # poc_a (baseline mode)
    "config_lam010_retune_tcn.yaml",   # tcn (baseline mode)
]

# Prior points on the λ sweep, for 3-way overlay.
SWEEP_PAIRS = {
    "config_lam010_retune.yaml": {
...
def train_all() -> None:
    ...
    _run("diagnostic_lam010_retune.py")


# ---------------------------------------------------------------------------
# Trajectory overlay — 3-point λ sweep (0, 0.01, 0.10)
# ---------------------------------------------------------------------------


def compare_trajectories() -> None:
    """Overlay λ=0.10 circular-loss + std_ratio trajectories on λ=0 and λ=0.01."""
    import matplotlib
    ...
    md_path = out_dir / "lam010_retune_report.md"
    with open(md_path, "w") as f:
        ...
        f.write(
            "- **IMPROVED, not fully stable**: fewer unhealthy epochs than λ=0.05 "
            "but not yet clean — λ alone may not be sufficient for this "
            "architecture/head; consider architecture-level fixes.\n"
        )
        f.write(
            "- **STILL UNHEALTHY**: raising λ to {0} did not fix it — check "
            "diagnostic_lam010_retune.py's prediction-perturbation trace before "
