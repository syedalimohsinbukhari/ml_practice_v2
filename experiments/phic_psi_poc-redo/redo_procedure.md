...
- **2.6 [CORRECTED — new derivation needed]** Inference-time reconstruction. The original plan's branch-ambiguity analysis (`planning_files/phic_psi_implementation_plan_v4.md` §2.6/A.4) found "2-fold for φc, 4-fold for ψ" when recovering individual angles from `combo_A`/`combo_B` built on `φc±2ψ`. That fold count is a property of *that* formula — doubling φc's own contribution changes the map. `complex_mul(z_φc, z_φc)` is a 2-to-1 map (both `φc` and `φc+π` produce the same `2φc`-angle vector), so recovering φc from a `2φc`-based combo carries an *extra* branch beyond the original count. This redo must re-derive the exact fold count and the joint-consistency re-encoding filter (same mandatory check as the original: re-encode each candidate and discard any that don't reproduce both `combo_A` and `combo_B`) before writing any reconstruction code — do not reuse the original's "2-fold/4-fold" numbers.
