...
### A.8 Reconstruction branch-handling — corrected, derive before coding

Given predicted (or true) `combo_A = (2φc+2ψ)`-angle and `combo_B = (2φc−2ψ)`-angle vectors:

```
z_prod  = complex_mul(combo_A, combo_B)        # angle = 4*phi_c
z_ratio = complex_mul_conj(combo_A, combo_B)   # angle = 4*psi
```

Recovering `φc` from a `4φc`-angle vector is a 4-to-1 map (candidates at `φc, φc+π/2, φc+π, φc+3π/2`); recovering `ψ` from `4ψ` is likewise 4-to-1 with `ψ`'s own π-periodicity folded in — **re-derive the exact fold count and enumerate every candidate explicitly** before writing reconstruction code (this differs from the original's "2-fold φc / 4-fold ψ" count, which was for `φc±2ψ`'s `z_prod` angle of `2φc`, not `4φc`). As in the original, the mandatory filter is: re-encode every `(φc_candidate, ψ_candidate)` pair via A.2 and keep only those that reproduce **both** `combo_A` and `combo_B` — this is a validation-script-only technique (needs ground truth to pick the closest candidate), not a real-inference reconstruction method.
