#!/usr/bin/env python
"""Plot history.csv and diagnostics.csv from a PoC redo training run.

Copied verbatim in logic from ``experiments/phic_psi_poc/plot_poc.py`` —
no formula-specific code here at all (reads only CSVs). Thin proxy that
imports ``plot_history`` and ``plot_diagnostics`` from ``scripts.plot_run``
— no logic duplication.  Reads only CSV files; does not load any model or
allocate GPU memory.

Examples::

    python experiments/phic_psi_poc-redo/plot_poc.py \\
        experiments/phic_psi_poc-redo/config_poc.yaml
    python experiments/phic_psi_poc-redo/plot_poc.py \\
        experiments/phic_psi_poc-redo/config_poc.yaml 20260713_141208
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "config",
        help="config name such as 'poc', or path such as experiments/phic_psi_poc-redo/config_poc.yaml",
    )
    parser.add_argument(
        "run_folder",
        nargs="?",
