...
\begin{figure}[tbp]
    ...
    \caption[Chirp-mass recovery (positive control)]{Chirp-mass recovery
    on the validation set for the four Run~7 models ($R^2 = 0.93$--0.96).
    Together with merger time, SNR, and sky position, these positive
    controls establish that the shared trunk and training pipeline learn
    strain-derived parameters when the information is present.}
...
\begin{figure}[tbp]
    ...
    \caption[Predicted $\varphi_c$ distributions]{Predicted $\varphi_c$
        distributions (color) against the uniform truth (grey) for the four
        Run~7 models. Every model collapses to one or a few preferred angles
        ($r_{\mathrm{circ}} = 0.43$--0.99) while angular MAE sits at the null
        $\pi/2 \approx 1.571$ --- optimal constant-prediction behavior for an
...
\begin{table}[tbp]
    ...
    \begin{tabular}{@{}lll@{}}
        ...
        Gradient path dead                  & perturbation $+$ gradient-chain trace & path healthy; an $89\times$ prediction-sensitivity asymmetry vs a scalar head flagged as unresolved (\S\ref{sec:phicpsi:threats})                                                                    \\
...
\begin{figure}[tbp]
    ...
    \caption[$\lambda = 0$ ablation]{$\lambda = 0$ ablation for poc\_a and
    tcn: validation/training circular loss and validation std\_ratio for
        $\varphi_c$ (top) and $\psi$ (bottom); $\lambda = 0$ (solid) vs the
        $\lambda = 0.01$ references (dashed). Removing the penalty eliminates
        the upward validation-loss drift (e.g.\ $+0.0010$ vs $+0.0249$ for
        poc\_a $\varphi_c$) but lets $\lVert\mathbf{v}\rVert$ diverge
        (std\_ratio $> 140$ at peak). Circular loss remains at $\approx 1.0$
...
\item \textbf{An unexplained $89\times$ sensitivity asymmetry.}
A single-step perturbation probe found $\varphi_c$ predictions ${\sim}89\times$ more sensitive (relative change $1.6 \times 10^{-2}$ vs $1.8 \times 10^{-4}$) than a scalar head's.
The multi-step trace designed to resolve this was gated, by pre-registration, behind a stability gate that never passed, and has therefore never run.
It is a flagged unknown; a one-step snapshot cannot distinguish learning from oscillation around a constant.
...
(ii)~A \textbf{standalone multi-step perturbation trace} to retire the $89\times$ asymmetry independently of the retired $\lambda$-gate; a decoupled script (\texttt{perturbation\_trace\_standalone.py}, targeting the Run~7 checkpoints) has been prepared and awaits execution.
(iii)~An \textbf{architecture-level attack on the std\_ratio instability} for the two uninterpretable pairs (the pre-registered $\lambda$-sweep's named next lever), or an explicit decision to close that thread; alongside it, a finer freshly pre-registered $\lambda$ mini-sweep over $[0.02, 0.08]$, motivated by the peaked $\lambda$-response observed in \S\ref{sec:phicpsi:prereg}.
(iv)~A \textbf{posterior-estimation reformulation}: the natural follow-on from ``point estimation is hopeless'' is not resignation but a conditional-density head (e.g.\ a von Mises mixture over the well-constrained combination), which the present chapter's null both motivates and baselines.
