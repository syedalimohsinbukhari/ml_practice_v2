...
## Dated finding (2026-09-09): combo formula uses the wrong phase convention — should be `2φc ± 2ψ`, not `φc ± 2ψ`

**Trigger:** external reviewer flagged `φc ± 2ψ` (the combo_A/combo_B combination this entire investigation is built on) as the wrong term, proposing `2(φc ± ψ) = 2φc ± 2ψ` instead.

**Investigation.** Two lines of evidence were checked before accepting the correction.

First, an impact-assessment pass over this directory initially found `φc ± 2ψ` *internally* well-supported: cited in the thesis (Cutler 1994; Sathyaprakash & Schutz 2009, `thesis/chapter_phic_psi_degeneracy.md:18-19`), re-derived independently by `curriculum.py`'s toy antenna-pattern model (`h_plus/h_cross ∝ cos(2Φ+φc)` — φc singly, ψ doubled), and confirmed by a numerical harness check that already ran and passed (`prereq_checks.py:89-128`, sweeping ψ at fixed `φc+2ψ` and confirming `(R,δ)` stays constant at ι=0). All three, however, trace back to a single unverified source: `planning_files/phi_c_psi_degeneracy_poc.md` Sec. 1/4, which asserted `2ψ+φc` from a literature statement without cross-checking it against how this dataset's `coa_phase` is actually defined at injection time. None of the three is independent verification against the data-generation convention.

Second, the injection convention was traced directly. `src/gwml/gen_py_data_pipeline.md` documents this repo's `gen.py`; the sibling generator scripts on this machine (`ml-gw-search/mlgwsc-1/gen.py:202`, `ml-gw-search/extended_mass/gen.py:179`, same structural lineage) show `waveform_kwargs['coa_phase'] = angles[0]` passed straight into `pycbc.waveform.get_td_waveform(**waveform_kwargs)`. PyCBC's `coa_phase` is LALSimulation's `phiRef` — the **orbital** reference phase, range `[0, 2π]` (per the inline comment at `gen.py:262`). The approximant is IMRPhenomD, dominant (2,2) mode only (already established, `NOTES.md`'s v2 adversarial-review section, author-confirmed out-of-band). For a pure (2,2)-mode signal, the strain depends on the orbital reference phase through `e^{i·2·coa_phase}` — the textbook "waveform invariant under `coa_phase → coa_phase+π`" result — so `coa_phase`, as actually injected here, enters the waveform **doubled**, exactly like `ψ` does.

`curriculum.py:35-37` gives away the conflation directly: `h_plus = k1(ι)·cos(2·Φ + phi_c)`, where the docstring states `Φ` is *orbital* phase (so `2Φ` is the GW phase) — yet `phi_c` is added to `2Φ` *singly*. That only holds if `phi_c` is already a GW-frame/doubled quantity, which is the classical SPA/frequency-domain convention (matching the Cutler/Sathyaprakash-Schutz citations) — not what `coa_phase` denotes in the PyCBC/LALSimulation injection convention this dataset actually uses. The toy model, the harness check built on it, and the combo_A/combo_B construction in `trainer.py` all inherited this same unchecked convention.

**Conclusion: the correct degenerate combination for this dataset is `2φc ± 2ψ` = `2(φc ± ψ)`**, not `φc ± 2ψ`. The reviewer's correction stands.

**Scope of damage — not localized.** This invalidates the central null-result conclusion of the investigation as currently written (every combo_A/combo_B mode-collapse/null finding — Round 1, Runs 6–9b, λ-retune, verification Sections A–E — was tested against a combination that isn't actually the degenerate/well-constrained direction). The fix itself is small and mechanical:

- **RE-EDIT:** `curriculum.py:35-42` (the actual root — `h_plus`/`h_cross` need `2·phi_c`, not `phi_c`); `trainer.py:284-288` combo_A/B construction (needs `z_φc` doubled, e.g. via a period=π treatment or explicit squaring before `complex_mul`); `transform_utils.py` reconstruction math; `heads_spec.py:101`'s `coa_phase` period is a separate, deliberate call — the injected value does span the full `[0,2π)`, so the fix belongs in the combo construction, not necessarily the head's own PERIODIC period; `prereq_checks.py`, `validation_script.py`, `diagnostic_checks.py` plot labels; `planning_files/phic_psi_implementation_plan_v4.md` §A.2 and `phi_c_psi_degeneracy_poc.md` Sec. 1/4 stay frozen (dated addenda only, per this repo's convention) rather than rewritten in place.
- **RE-RUN:** `curriculum.py` + `prereq_checks.py` first (CPU-only, ~30–70 min) to regenerate `sweep_1_1_ratio_vs_iota.{csv,png,pdf}` and the curriculum weight under the corrected formula; then every SumDiffTrainer training run (Round 1, Runs 6–9b) on the lab GPU machine, since every `runs/phic_psi_*/` checkpoint was trained against the wrong combo target; then every downstream diagnostic/analysis script that reads those checkpoints (`analyse_predictions.py`, `diagnostic_checks.py`, `evaluate_poc.py`, `inclination_stratification.py`, `snr_stratification.py`, `bootstrap_ang_mae.py`, `perturbation_trace_standalone.py`, `plot_certified_memorization.py`).
- **RE-WRITE:** `NOTES.md`'s Run Log (mode-collapse/null-result findings), this log's own combo-related verdicts throughout, `results.md`, `std_ratio_trajectories.md`, `phic_psi_run7_comparison.md`, `assessment_lam0_ablation_2026-07-22.md`, `preregistration_lam_retune.md`, `phic_psi_closing_punch_list.md`, `run7_verification_plan.md`/`_rebuttal.md`, `reviewer_response_a3_2026-07-23.md`, the thesis chapter (`thesis/chapter_phic_psi_degeneracy.{md,tex}`) and `paper/paper1/006_results.tex`, `008_discussion.tex`, `009_conclusion.tex` — the "degeneracy is fundamental, network learns nothing" framing is unverified pending a rerun under the corrected combo, not confirmed to be wrong, just no longer established.

**Status: OPEN.** No code has been changed and no rerun has happened yet — this entry records the finding and its evidence chain per the living-docs convention; `experiment_index.md` and the thesis chapter still need a corresponding status sweep once the fix/rerun path is decided.
