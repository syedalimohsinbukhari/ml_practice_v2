"""SumDiffTrainer — φc/ψ degeneracy PoC redo custom trainer.

Copied from ``experiments/phic_psi_poc/trainer.py`` with one functional
change: ``_build_combo_vectors`` now doubles ``z_phic`` (via
``tf_double_angle``, formulae_reference.md A.1.5) before combining with
``z_psi``, so the combos represent ``2φc±2ψ`` instead of the original's
``φc±2ψ`` (A.2, corrected per A.6's injection-convention finding). No other
logic changes — the loss functions, magnitude penalty, curriculum
weighting, and metrics machinery are all formula-agnostic and correct as
written.
...
Two modes (controlled by the ``mode`` init parameter):
  ...
  ``"poc"`` (Run B)
      Circular loss on combo_A (2φc + 2ψ) and combo_B (2φc − 2ψ), with
...
class SumDiffTrainer(MultiHeadTrainer):
    ...
    def _build_combo_vectors(self, y_true, y_pred):
        ...
        combo_A_true, combo_A_pred : (N, 2) unit vectors at 2φc+2ψ
        combo_B_true, combo_B_pred : (N, 2) unit vectors at 2φc−2ψ
        ...
        # Build combos (A.2, corrected).
        # z_ψ already encodes 2ψ internally (PERIODIC, period=π), unchanged.
        # z_φc encodes φc directly (PERIODIC, period=2π, unchanged head) —
        # double it here (A.1.5) so the combo reflects 2φc, per A.6's
        # confirmed injection-convention finding. This is the one line that
        # differs from experiments/phic_psi_poc/trainer.py.
        z_2phic_true = tf_double_angle(z_phic_true)
        z_2phic_pred = tf_double_angle(z_phic_pred)
        combo_A_true = tf_complex_mul(z_2phic_true, z_psi_true)
        combo_A_pred = tf_complex_mul(z_2phic_pred, z_psi_pred)
        combo_B_true = tf_complex_mul_conj(z_2phic_true, z_psi_true)
        combo_B_pred = tf_complex_mul_conj(z_2phic_pred, z_psi_pred)
