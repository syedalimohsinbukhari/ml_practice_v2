...
Redo counterpart of ``experiments/phic_psi_poc/snr_stratification.py``, copied verbatim
in logic and repointed at this directory's own configs/checkpoints — same "redo every
check, even the ones not expected to change" rationale as inclination_stratification.py's
header (this experiment's own mandate: a null result only justifies itself if the checks
that could have found a signal were actually rerun, not assumed to transfer from the
original's wrong-formula checkpoints).

Section E of the original's Run 7 verification plan: if even the highest-SNR tercile
shows no improvement over baseline, that's physics evidence, not engineering.

Usage (on GPU machine — loads best.weights.h5, calls trainer.predict; not run locally
per this repo's CLAUDE.md CPU-only rule):
    python experiments/phic_psi_poc-redo/snr_stratification.py
...
def main():
    ...
    print("=" * 100)
    print("SNR-STRATIFIED ang_MAE — final epoch, all models, all periodic heads (REDO)")
    ...
    with open(md_path, "w") as f:
        f.write(f"# SNR-Stratified ang_MAE — All Periodic Heads (Redo)\n\n")
