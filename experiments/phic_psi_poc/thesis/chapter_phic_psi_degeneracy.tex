...
% chapter appendix).
%
% EDITORIAL NOTE -- remove before final submission.
% This chapter has been through four rounds of adversarial AI review
% (thesis/reviews/: unsuffixed files are round 1, *_v2.md files are
% round 2, kimi_ai_adversarial_review_v3.md is round 3, *_v4.md files
% are round 4 -- all three reviewers returned for round 4, whose
% feedback was overwhelmingly tonal rather than structural; the full
% item-by-item cross-check is thesis/reviews/v4_correction_checklist.md).
% All four rounds' fixes and new analyses are folded into the text
% below (Sec 1, Sec 2.1, Sec 6.2, Sec 6.6, Sec 6.7, Sec 7.1, Sec 8.2,
% Fig 8.1, Table 6.7, Sec 8.5, and framing changes throughout Sec 6.3,
% Sec 8.3, Sec 9). Four items remain CRITICAL FUTURE WORK that this
% text pass cannot resolve, so a future review pass -- human or AI --
% does not mistake the omission for an oversight: (1) a synthetic-data
% ablation to disentangle poc_b's curriculum design from the physical
% degeneracy; (2) a high-SNR (25-30) validation set or a Fisher-matrix
% bound, to rule out a sensitivity-floor confound distinct from
% degeneracy; (3) regenerating the dataset (or a small slice of it)
% from a version-controlled generator script, to close the
% waveform-provenance gap for good; (4) a 2D joint-distribution
% ("ridge test") check of predicted (phi_c-hat, psi-hat) pairs against
% saved checkpoints, raised in round 4, to directly test whether the
% network learned the degenerate manifold itself rather than nothing --
% inference-only but requires GPU access not available on this
% machine. A fifth item raised in round 2 -- either a fresh-holdout
% pre-registered replication of the A.3 perturbation trace, or a formal
% downgrade of A.3 from "closed" to "open" -- is now reflected directly
% in the chapter body: Sec 6.6 states A.3's closure as provisional,
% pending that replication, rather than asserting closure outright.
% Each of the four remaining items is a genuine, well-posed research
% task, not a text fix -- and each was judged, at this point in the
% project, to cost more in new training/data-generation effort than
% the marginal certainty it would buy, weighed against finishing the
% thesis. Recorded as scoped future work in experiment_index.md and
% NOTES.md (2026-07-23, "Round 2" and "Round 3" entries; 2026-07-24,
% "Round 4" entry) rather than silently dropped, and -- as of the
% consistency audit of 2026-07-24 -- all four are also carried in the
% chapter body itself as Sec 8.5 items (iv)-(vii), so this note can be
% removed without orphaning any of them.
...
Since strain alone does not recover $\varphi_c/\psi$ under point estimation, but \S\ref{sec:phicpsi:prereq}'s analytic study shows the well-constrained combination \emph{is} recoverable given inclination, the natural next step is to condition the network on $\iota$ directly --- scoped as future work in \S\ref{sec:phicpsi:future} and taken up in the chapter that follows this one.
...
No model in this chapter takes $\iota$, or any function of it, as an input at any stage; $\iota$ appears here only as a control \emph{output} head trained on raw strain, exactly like $\varphi_c$ and $\psi$ (\S\ref{sec:phicpsi:circular}).
...
This detail is not preserved as HDF5 metadata or in any generation script retained in this repository (the generator itself lives outside the tracked pipeline); it is recorded here from the authors' knowledge of how the dataset was produced, an honest provenance gap flagged in \S\ref{sec:phicpsi:threats}.
...
\begin{table}[tbp]
    ...
    \begin{tabular}{@{}lllc@{}}
        ...
        & dilations 1--512, 64 filters, receptive field $\approx$ full window~\cite{bai2018tcn} &                   &            \\
        ...
        \texttt{resnet1d}        & ResNet-style, 3 stages, filters 64/128/256, dilated second blocks~\cite{he2016resnet} & GAP $\oplus$ GMP & 512 \\
        \bottomrule
    \end{tabular}
\end{table}
...
Separately, and only as a secondary point, the direction of the risk is asymmetric: if this recollection were wrong and higher-order modes were in fact present, the degeneracy would be only approximately exact even face-on, which would \emph{sharpen} rather than weaken the null (a network failing to exploit even a HOM-broken degeneracy is stronger evidence of no recoverable signal) --- but this does not excuse the provenance gap, which is a threat to the chapter's reproducibility independent of which way it would cut if resolved.
...
(iv)~A \textbf{ridge-structure check}: a 2D joint scatter of the certified models' predicted $(\hat\varphi_c, \hat\psi)$ pairs, to test directly whether the network learned the degenerate combination manifold itself (which would appear as a diagonal ridge) rather than the collapsed or spread patterns already documented in Fig.~\ref{fig:phicpsi:hist} --- raised in the fourth adversarial-review round; inference-only against saved checkpoints, no retraining required, but not run here because no script or output artifact yet backs it (the same artifact-traceability standard \S\ref{sec:phicpsi:threats} sets for the current text applies equally to future additions).
...
(vi)~A \textbf{sensitivity-floor check}: a high-SNR (25--30) validation set, or a Fisher-matrix bound on the achievable $\varphi_c/\psi$ accuracy at this population's SNR range, to rule out the possibility that the null reflects a detection-scale sensitivity floor rather than the degeneracy as such.
...
Three chapter numbers are \emph{derived} from their artifacts rather than printed in them, and are flagged here under the same traceability rule.
...
\section*{Suggested citation keys (to resolve against thesis bibliography)}

\texttt{abbott2016gw150914} (first detection), \texttt{cutler1994gw} (waveform/PE foundations), \texttt{sathyaprakash2009physics} (GW physics review), \texttt{veitch2015lalinference} (Bayesian PE), \texttt{george2018deep} (first DL for GW), \texttt{gabbard2022vitamin}, \texttt{dax2021dingo} (neural posterior estimation), \texttt{kendall2018multi} (uncertainty weighting), \texttt{mardia2000circular} (circular statistics), \texttt{fisher1953dispersion} (vMF), \texttt{bai2018tcn}, \texttt{he2016resnet}, \texttt{fawaz2019inceptiontime}, \texttt{vaswani2017attention} (architectures), \texttt{nosek2018preregistration} (pre-registration).
