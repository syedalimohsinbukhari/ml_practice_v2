#!/usr/bin/env python
"""Standalone validation script for the φc/ψ PoC redo's transform utilities.

Copied from ``experiments/phic_psi_poc/validation_script.py``, updated for
the corrected ``2φc±2ψ`` combo (formulae_reference.md A.2/A.8). Tests the
complex-multiplication utilities and reconstruction logic using synthetic
known-answer data.  Does NOT require a trained model or GPU.

Run::

    python experiments/phic_psi_poc-redo/validation_script.py
...
5.  Reconstruction round-trip (A.8) — known (φc, ψ) recovered among
    jointly-consistent candidates, using the corrected 2φc+2ψ/2φc−2ψ combo.
6.  Known alias group — for true (φc₀,ψ₀), the full jointly-consistent set
    is {(φc₀,ψ₀), (φc₀+π,ψ₀), (φc₀+π/2,ψ₀+π/2), (φc₀+3π/2,ψ₀+π/2)}
    (verified numerically over 3000 random trials, 2026-09-14, before
    being written into this test — see formulae_reference.md A.8's
    correction note for why hand-derived claims here get verified first).
7.  Joint-consistency filter — exactly 4 of 8 raw candidates pass (fold
    counts differ from the original: φc is 4-fold, ψ is 2-fold, vs. the
    original's 2-fold/4-fold — the 4-of-8 total is the same by coincidence).
8.  ``w_iota`` — w=1 at edge-on (cos ι=0), w=0 at face-on (|cos ι|=1).
    (``w_iota_default`` is a generic fallback, not formula-dependent —
...
# Local unqualified imports (not experiments.phic_psi_poc.transform_utils/
# curriculum, which would point at the original, wrong-formula modules —
# and "phic_psi_poc-redo" isn't a valid dotted package name anyway, since
# Python identifiers can't contain hyphens). Resolves to this directory's
# own modules via the running script's own directory being on sys.path,
# same pattern trainer.py/prereq_checks.py use.
...
def test_reconstruction_roundtrip():
    ...
    for i in range(n):
        ...
        # Build combo_A and combo_B from known truth (corrected, A.2)
        z_phic = np.array([np.sin(phic_true), np.cos(phic_true)])
        z_2phic = double_angle(z_phic)
        # ψ PERIODIC encoding: z_ψ = [sin(2ψ), cos(2ψ)] because period=π
        z_2psi = np.array([np.sin(2.0 * psi_true), np.cos(2.0 * psi_true)])

        combo_A = complex_mul(z_2phic, z_2psi)  # 2φc + 2ψ
        combo_B = complex_mul_conj(z_2phic, z_2psi)  # 2φc − 2ψ
...
# ---------------------------------------------------------------------------
# Test 6 — Known alias group (corrected for 2φc±2ψ)
# ---------------------------------------------------------------------------


def test_alias():
    print("\n── Test 6: Known alias group ──")

    # For true (φc₀,ψ₀), the full jointly-consistent set is:
    #   {(φc₀,ψ₀), (φc₀+π,ψ₀), (φc₀+π/2,ψ₀+π/2), (φc₀+3π/2,ψ₀+π/2)}
    # This differs from the original's single (φc+π, ψ+π/2) alias — the
    # fold structure flipped (φc 4-fold / ψ 2-fold here, vs. 2-fold/4-fold
    # originally). Verified numerically over 3000 random trials before
    # being written here (see formulae_reference.md A.8's correction note
    # — a hand-derived claim about this same reconstruction was wrong
    # earlier the same day, so this one was checked computationally first,
    ...
    for i in range(n):
        ...
        combo_A = complex_mul(z_2phic, z_2psi)
        combo_B = complex_mul_conj(z_2phic, z_2psi)

        phic_cands, psi_cands, _ = reconstruct_phic_psi(
            combo_A[np.newaxis, :], combo_B[np.newaxis, :]
        )

        for d_phic_offset, d_psi_offset in alias_offsets:
            alias_phic = (phic_true + d_phic_offset) % (2.0 * np.pi)
            alias_psi = (psi_true + d_psi_offset) % np.pi
            ...
            for k in range(phic_cands.shape[-1]):
                ...
                if abs(d_phic) < 1e-8 and abs(d_psi) < 1e-8:
                    found_alias = True
                    break

            if not found_alias:
                check(
                    f"alias (φc+{d_phic_offset:.3f}, ψ+{d_psi_offset:.3f}) recovered",
                    False,
                    f"candidates: φc={phic_cands[0]}, ψ={psi_cands[0]}",
                )
                return
    check("all 20 trials: full 4-member alias group recovered", True)
...
def test_analytical_model_iota_zero():
    ...
    # For each (a,b) pair, fix one const_line = 2φc + 2ψ, then sweep ψ
    ...
    for a, b in sky_coeffs:
        ...
        const_line = 2.0 * phic_base + 2.0 * psi_base

        # Sweep ψ, keeping 2φc + 2ψ = const_line
        R_group = []
        delta_group = []
        n_sweep = 15
        for psi in np.linspace(0.0, np.pi, n_sweep, endpoint=False):
            phic = ((const_line - 2.0 * psi) / 2.0) % (2.0 * np.pi)
