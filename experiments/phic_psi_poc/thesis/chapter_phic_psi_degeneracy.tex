...
\begin{table}[tbp]
    ...
    \begin{tabular}{@{}lll@{}}
        ...
        Gradient path dead                  & perturbation $+$ gradient-chain trace & path healthy; an $89\times$ prediction-sensitivity asymmetry initially flagged, provisionally closed by the standalone multi-step trace pending instrument calibration (\S\ref{sec:phicpsi:asymmetry})                                                                    \\
...
\begin{table}[tbp]
    \centering
    \caption{Standalone multistep perturbation trace. net/sum is the ratio of net displacement to summed per-step displacements (random-walk reference $1/\sqrt{25} = 0.20$; directional drift $\to 1$).
        $\Delta$ circ.\ loss is the probe circular-loss change over the 25 steps.}
    \label{tab:phicpsi:trace}
    \small
    \begin{tabular}{@{}lccc@{}}
        \toprule
        Model & $\varphi_c$: cos-sim / net-sum / $\Delta$loss & $\psi$: cos-sim / net-sum / $\Delta$loss & mchirp: cos-sim / net-sum \\
        \midrule
        poc\_a         & $+0.93$ / 0.29 / $-0.017$ & $+0.96$ / 0.59 / $+0.048$ & $+0.60$ / 0.04 \\
        poc\_b         & $+0.96$ / 0.39 / $-0.013$ & $+0.80$ / 0.23 / $+0.004$ & $+0.54$ / 0.04 \\
        tcn            & $+0.94$ / 0.93 / $-0.010$ & $+0.87$ / 0.39 / $+0.051$ & $+0.59$ / 0.03 \\
        cnn\_attention & $+0.66$ / 0.40 / $-0.046$ & $+0.64$ / 0.32 / $+0.029$ & $+0.51$ / 0.16 \\
        \bottomrule
    \end{tabular}
\end{table}
...
The closure is nonetheless \textbf{provisional}, for two reasons surfaced in internal review --- and stated here rather than smoothed over, in keeping with the discipline of the methodological-lessons subsection below.
First, \emph{the positive control failed}: mchirp, the best-learned head in the investigation ($R^2 \approx 0.96$ throughout), read AMBIGUOUS in all four models, its net/sum (0.03--0.16) sitting at the random-walk reference.
...
Because per-epoch checkpoints were not saved, the calibration uses a fresh initialization plus a ${\sim}$one-epoch warmup rather than a loaded early checkpoint.
Second, \emph{per-case discipline on the two directional periodic cases}: poc\_a/$\psi$ fails the pre-stated two-part escalation rule outright --- its probe loss \emph{increased} ($+0.048$) --- but tcn/$\varphi_c$ nominally triggers it (probe loss $-0.010$ with net/sum 0.93, the closest number to the theoretical ceiling in the whole table).
The strongest per-case dismissal is arithmetic: 25 steps is ${\sim}0.13$ epochs of gradient, so a real angular improvement at this rate would imply epoch-scale loss movement two orders of magnitude larger than the same head's flat 80-epoch history shows.
But the accompanying ``inside sampling noise'' claim compared against the probe's \emph{marginal} standard error (${\sim}0.03$), where the correct comparator for a before/after change on the same fixed samples is the \emph{paired} standard error --- not recoverable from the recorded output, and added to the instrument for the calibration rerun.

A.3 therefore stands as \emph{provisionally closed}: movement-without-angular-learning is the best-supported reading, pending an early-training calibration run that must show mchirp directional-early/ambiguous-late for the final-stage verdicts to be trusted at face value.
Should tcn/$\varphi_c$'s directionality survive that calibration as a paired-significant angular effect, it would be the most interesting number in the entire investigation and would be escalated per the pre-stated rule, not filed.
...
\item \textbf{The perturbation-trace instrument is uncalibrated at converged checkpoints.}
Its positive control (mchirp) read AMBIGUOUS on all four epoch-79 checkpoints (\S\ref{sec:phicpsi:asymmetry}), so the trace's periodic-head verdicts --- including the nominal tcn/$\varphi_c$ escalation trigger --- are provisional until the early-training calibration run lands.
If calibration fails (mchirp ambiguous even early), the trace methodology is unsound and A.3 reverts to open; if it passes but tcn/$\varphi_c$'s directionality persists as a paired-significant angular effect, that case escalates.
...
(0)~The \textbf{perturbation-trace calibration run} (\S\ref{sec:phicpsi:asymmetry}): the \texttt{early} stage of \texttt{perturbation\_trace\_standalone.py} (fresh init + ${\sim}$one-epoch warmup, per-sample paired statistics), which either validates the provisional A.3 closure, invalidates the instrument, or escalates tcn/$\varphi_c$ --- cheap, and first in line because the asymmetry subsection's status depends on it.
