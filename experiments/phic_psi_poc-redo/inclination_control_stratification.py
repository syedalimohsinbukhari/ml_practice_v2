...
Redo counterpart of ``experiments/phic_psi_poc/inclination_control_stratification.py``,
copied verbatim in logic and repointed at this directory's own configs/checkpoints — see
``experiment_index.md``'s "carried over unchanged" convention, and the sibling script
``inclination_stratification.py``'s header for why this is rerun rather than assumed to
still hold: the redo's mandate is to redo every check the original performed, including
this one, whose result isn't expected to change (the banding scheme itself is not
formula-dependent) but whose *rerun* is still required to justify the redo's own null
result rather than borrowing the original's.

Adversarial-review follow-up (v2, from the original): the phi_c/psi inclination
stratification uses the known-uninformative inclination (iota) head as a same-model noise
...
even into a head we know is well-learned and non-angular (chirp mass)? If chirp-mass
MAE/R^2 stays essentially flat across bands, that rules out "the banding itself is noisy
at these N" as an explanation for the iota head's band-to-band swings, strengthening (not
proving) the noise-floor argument in inclination_stratification.py's output.
...
Usage (on GPU machine — loads best.weights.h5, calls trainer.predict; not run locally
per this repo's CLAUDE.md CPU-only rule):
    python experiments/phic_psi_poc-redo/inclination_control_stratification.py
...
def main() -> None:
    out_dir = Path("experiments/phic_psi_poc-redo/inclination_output")
    out_dir.mkdir(parents=True, exist_ok=True)
    ts = _setup_logging(out_dir)

    print("=" * 100)
    print("INCLINATION-STRATIFIED CHIRP-MASS MAE/R^2 -- scalar-control check (REDO)")
    ...
    with open(md_path, "w") as f:
        f.write("# Inclination-Stratified Chirp-Mass MAE/R^2 -- Scalar-Control Check (Redo)\n\n")
        f.write(f"**Generated**: {ts}\n")
        f.write(f"**Bands**: face-on |cos(iota)| > {FACE_ON_COS_IOTA}, "
                f"edge-on |cos(iota)| < {EDGE_ON_COS_IOTA}, mixed in between "
                f"(matches original thesis chapter Section 3 / inclination_stratification.py)\n")
        f.write(f"**Validation samples**: {N_SAMPLES}\n\n")

        f.write("## Key question\n\n")
        f.write("The original thesis chapter's Section 6.7 uses the known-uninformative "
                "inclination head as a same-model noise floor for the phi_c/psi inclination "
                "stratification. This script checks the companion question, on the redo's own "
                "checkpoints: does the face-on/mixed/edge-on banding scheme itself inject "
                "spurious MAE/R^2 variance, even into a head (chirp mass) we already know is "
                "well-learned and non-angular? If the spread here is small relative to the "
                "phi_c/psi deviations in inclination_stratification.py's output, banding-"
        ...
        f.write("Small band-to-band MAE/R^2 spread here, relative to the phi_c/psi edge-on "
                "deviations in inclination_stratification.py's output, supports treating those "
                "deviations as head-specific (iota-noise-floor-scale) rather than an artifact "
                "of slicing the validation set into unequal, differently-composed bands.\n")
