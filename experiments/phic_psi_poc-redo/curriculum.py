"""Curriculum weight function w(ι) for the φc/ψ degeneracy PoC redo.

Implements Step 1.2 of ``redo_procedure.md``: an analytical harness that
sweeps ι across [0, π/2], computes the Jacobian condition number of the
(φc, ψ) → (R, δ) map at each inclination (averaged over random sky
...
Toy detector model is from ``formulae_reference.md`` A.5 — copied from
``experiments/phic_psi_poc/curriculum.py`` with one correction: ``h_plus``/
``h_cross`` now add ``2*phi_c`` (not ``phi_c``) to the orbital-phase term,
per A.6's confirmed injection-convention finding. Do NOT re-derive the sign
conventions beyond that single documented change.
"""
...
# ---------------------------------------------------------------------------
# formulae_reference.md A.5 — Toy antenna-pattern / detector-response model
# (corrected: h_plus/h_cross now use 2*phi_c, see docstrings below)
# ---------------------------------------------------------------------------
...
def h_plus(phi_c: np.ndarray, iota: np.ndarray, Phi: np.ndarray) -> np.ndarray:
    ...
    Corrected (A.5): adds ``2*phi_c`` to the GW phase ``2*Phi``, not
    ``phi_c`` — ``coa_phase`` is the orbital reference phase (A.6) and
    enters the dominant (2,2)-mode strain doubled, same as ``2*Phi`` itself.
    """
    return _k1(iota) * np.cos(2.0 * Phi + 2.0 * phi_c)


def h_cross(phi_c: np.ndarray, iota: np.ndarray, Phi: np.ndarray) -> np.ndarray:
    """h_cross strain component at orbital phase Phi. Corrected, see h_plus."""
    return _k2(iota) * np.sin(2.0 * Phi + 2.0 * phi_c)
...
def derive_w_iota(
    ...
    **Fit method:**  Uses the raw empirical curve directly via linear
    interpolation on cos²ι, clamped to [0, 1] — same choice the original
    made (an unconstrained polynomial fit was explicitly rejected there for
    producing negative weights and residuals larger than the function's own
    range; unrelated to the combo-formula bug, so that decision still
    holds).  The interpolation table is returned alongside the smooth fit
    for inspection.

    **The numeric output of this function differs from the original's**
    (``experiments/phic_psi_poc/curriculum.py``) even though the recipe is
    identical, because ``h_plus``/``h_cross`` — and therefore the Jacobian
    — are now built from the corrected A.5 formula.  Do not reuse the
    original's ``w = 1 − cos²ι`` fallback or its empirical fit numbers as
    if they still apply.
    ...
    fit_info = (
        f"Interpolation on cos²ι (linear), corrected h_plus/h_cross (A.5).  "
