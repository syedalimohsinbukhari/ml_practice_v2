#!/usr/bin/env python
"""Train Run A or Run B for the φc/ψ degeneracy PoC redo.

Copied verbatim in logic from ``experiments/phic_psi_poc/train_poc.py`` —
no functional changes. This file has no formula-specific code; it's pure
orchestration (config/data loading, trainer construction, fit call). The
combo formula correction lives entirely in ``trainer.py``/``curriculum.py``/
``transform_utils.py``, which this file imports locally (unqualified —
resolves to this directory's own copies).
...
Usage::
    ...
    python experiments/phic_psi_poc-redo/train_poc.py \\
        experiments/phic_psi_poc-redo/config_poc.yaml

    # Run A (baseline — independent φc/ψ, same circular loss)
    python experiments/phic_psi_poc-redo/train_poc.py \\
        experiments/phic_psi_poc-redo/config_baseline.yaml
"""
