...
An \textbf{inclination-stratified breakdown of the existing Run~7 checkpoints} (\texttt{inclination\_stratification.py}, prepared alongside this revision, mirroring the SNR-tercile analysis of \S\ref{sec:phicpsi:snr} but binning on $|\cos\iota|$ against the face-on / mixed / edge-on bands of \S\ref{sec:phicpsi:prereq}) directly tests the one population slice never separately reported: if edge-on systems --- where the analytic degeneracy is weakest --- already show measurably lower ang\_MAE than face-on systems in the existing data, that sharpens the null's boundary without requiring new training; if flat, it corroborates the null's uniformity across the one axis not yet checked.
The script requires only a CPU-light inference pass over the four $\lambda$-matched checkpoints and no retraining; it is prepared but not yet executed, and its output belongs in a revision of \S\ref{sec:phicpsi:results} once run.
