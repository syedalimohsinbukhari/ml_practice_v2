...
- **2.6 [CORRECTED — derived, then corrected again after implementation, 2026-09-14]** Inference-time reconstruction. The original plan's branch-ambiguity analysis (`planning_files/phic_psi_implementation_plan_v4.md` §2.6/A.4) found "2-fold for φc, 4-fold for ψ" for `φc±2ψ`'s reconstruction — a property of that formula, not reusable here. Under `2φc±2ψ`, the fold counts swap: **ψ is 2-fold ambiguous** (candidates `ψ₀`, `ψ₀+π/2`), **φc is 4-fold ambiguous** (candidates `φc₀+k·π/2`, k=0..3), and exactly 4 of the naive `4×2=8` candidate pairs are jointly consistent, always. A same-day review proposed a fixed closed-form parity rule (`k≡j mod 2`) to pick out those 4 without brute force — **implementing and testing it immediately disproved it**: a 20,000-trial numerical sweep showed which parity pattern applies is ~50/50 and data-dependent, not fixed, because it depends on an integer that `arctan2`'s mod-2π reduction destroys. See `formulae_reference.md` §A.8 for the full correction record (kept, not erased, per this repo's frozen-vs-living-docs convention). **Net: no shortcut — the reconstruction code brute-forces all 8 candidates and re-encode-checks every time**, same approach the original always used, just with the corrected branch counts. `transform_utils.py`'s `reconstruct_phic_psi` implements this, verified against 5,000 random trials (100% true-value recovery, always exactly 4 survivors).
