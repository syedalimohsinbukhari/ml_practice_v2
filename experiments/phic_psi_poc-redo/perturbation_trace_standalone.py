...
Redo counterpart of ``experiments/phic_psi_poc/perturbation_trace_standalone.py``,
copied verbatim in logic and repointed at this directory's own configs/checkpoints —
same "redo every check the original ran" rationale as inclination_stratification.py's
header. Particularly relevant here: this session's diagnostic_logvar_gate.py found a
std_ratio false-positive at λ=0.10 (a mode-collapsed constant output that happened to
have healthy population variance) that only a scatter-plot check caught — this trace
is a more systematic instrument for exactly that ambiguity (directional-but-slow
learning vs. oscillation around a constant), and running it against the redo's own
checkpoints checks whether it would have caught the same thing mechanically.

Un-gated version of the trace embedded in diagnostic_lam005_retune.py /
diagnostic_lam010_retune.py (originals) — this redo's `diagnostic_logvar_gate.py`
does not embed an equivalent trace, so this standalone script is the only place
that check exists for the redo. Runs against existing checkpoints (or a fresh
...
Usage (lab GPU machine only — do not run on the T530):
    python experiments/phic_psi_poc-redo/perturbation_trace_standalone.py          # final
    python experiments/phic_psi_poc-redo/perturbation_trace_standalone.py early    # calibration
...
def main():
    ...
    print("=" * 90)
    print(f"STANDALONE MULTI-STEP PERTURBATION TRACE (A.3, REDO) — stage={stage}, "
    ...
    with open(md, "w") as f:
        f.write(f"# Standalone Multi-Step Perturbation Trace (A.3, Redo) — stage: {stage}\n\n")
        if stage == "early":
