...
Inclination therefore reads statistically distinguishable from the shuffled null in one model under correction, and distinguishable only uncorrected in the other three --- but \S\ref{sec:phicpsi:inclination} shows this reflects partial mode collapse, not a recoverable per-sample signal, in every case with an effect size well below this study's own materiality floor of 0.10~rad.
...
But neither is it a small, real, per-sample-recoverable signal, the reading originally given here.
...
Tellingly, the ranking tracks each model's own $\varphi_c$/$\psi$ collapse severity: \pocA/\pocB (COLLAPSE-graded on $\varphi_c$/$\psi$) show the highest inclination $\text{circ}_r$, \Tcn/\cnnAttn (not collapsed there) the lowest --- consistent with a shared training-dynamics origin across a model's periodic heads, not head-specific recoverable physics.
