...
Usage::
    ...
    python experiments/phic_psi_poc-redo/prereq_checks.py
...
def _check_sign_combination(
    ...
    print(f"  ✓ Harness check passed: (R,δ) constant at ι=0 along 2φc+2ψ=const "
    ...
    def _correlations_at_iota(iota_val, rng_local):
        ...
        for a, b in sky_coeffs:
            ...
            cA_fixed = rng_local.uniform(0.0, 6.0 * np.pi)
            cB_vals = np.linspace(0.0, 6.0 * np.pi, n_sweep)
            R_vals, delta_vals = [], []
            for cB in cB_vals:
                phic = (cA_fixed + cB) / 4.0
                psi = (cA_fixed - cB) / 4.0
                sig = detector_signal(psi, phic, iota_val, a, b, Phi)
                R, delta = project_to_R_delta(sig, Phi)
                R_vals.append(R)
...
def main():
    ...
    if not args.skip_w_derivation:
        ...
        from curriculum import derive_w_iota  # local import, see note above
    ...
    if not args.skip_histogram:
        print("\n── Step 1.6: cos ι histogram ──")
        from curriculum import histogram_cos_iota  # local import, see note above
