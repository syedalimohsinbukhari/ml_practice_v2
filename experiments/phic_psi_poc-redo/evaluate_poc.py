#!/usr/bin/env python
"""Evaluate a trained PoC redo checkpoint: metrics CSV + scatter and residual plots.

Copied verbatim in logic from ``experiments/phic_psi_poc/evaluate_poc.py``
— no formula-specific code here (evaluation/plotting only; the combo
formula lives in ``trainer.py``, imported locally via ``train_poc``).
...
Usage::
    ...
    python experiments/phic_psi_poc-redo/evaluate_poc.py \\
        experiments/phic_psi_poc-redo/config_poc.yaml --split validation
"""
