...
### Stage 2b (i) — GPU/eval work (✅ eleven of twelve done 2026-09-18)

Each script CPU-wiring-verified before handoff (config resolution, `best.weights.h5`/`transforms.json` presence, pure-math sanity on synthetic values). No ordering dependency between these nine — each reads Stage 2's checkpoints independently and writes to its own output directory — **except `perturbation_trace_standalone.py`, noted separately below.**

| Command | Runs | Status |
|---|---|---|
| `python experiments/phic_psi_poc-redo/inclination_stratification.py` | Loads each of the 4 certified models' `best.weights.h5`, predicts on 5,000 validation samples, bins `coa_phase`/`polarization_angle`/`inclination` ang_MAE by face-on/mixed/edge-on `\|cos ι\|` bands | done → [`inclination_output/`](inclination_output/) — no model clears the 0.10 rad floor |
| `python experiments/phic_psi_poc-redo/inclination_control_stratification.py` | Same bands, applied to `mchirp` (known-good, non-angular) as a control on whether the banding itself injects spurious variance | done → [`inclination_output/`](inclination_output/) — small spread, rules out banding artifacts |
| `python experiments/phic_psi_poc-redo/snr_stratification.py` | Same 4 models, ang_MAE binned by SNR tercile instead of inclination (Section E of the original's verification plan) | done → [`snr_output/`](snr_output/) — same null story |
| `python experiments/phic_psi_poc-redo/analyse_predictions.py` | Comprehensive prediction-distribution analysis (Table 4's COLLAPSE grade is the systematic version of this session's scatter-plot mode-collapse check); `.pdf` saving added to all 3 plot functions | done → [`analysis_output/`](analysis_output/) — **confirms `poc_a`/`poc_b` mode collapse** (near-delta-function histograms) |
| `python experiments/phic_psi_poc-redo/diagnostic_checks.py` | 5 of the original's 7 checks (Checks 5/7, tanh-saturation, intentionally omitted — inapplicable, this redo uses `activation="linear"` from day 1); Check 6's gradient chain corrected to include the `tf_double_angle` step the original's version lacked | done → `diagnostic_output/diagnostic_checks_20260916_113913.log` — **gradient chain healthy end-to-end; rules out an implementation bug** |
| `python experiments/phic_psi_poc-redo/bootstrap_ang_mae.py` | Shuffle-null bootstrap significance test (10,000 iterations) on `coa_phase`/`polarization_angle`/`inclination` ang_MAE, all 4 certified models — ungated, broader than `preregistration_lam_retune.md`'s Steps 1–3 | done → [`bootstrap_output/`](bootstrap_output/) — **`coa_phase`/`polarization_angle`: formal null, all 4 models. `inclination`: significant but sub-floor.** |
| `python experiments/phic_psi_poc-redo/rose_plot_residuals.py` | Bare-minimum polar histogram (rose plot) of wrap-aware `pred − true` residual per periodic head/model — a collapsed constant predictor shows as a flat ring, real recovery as a spike near zero. Prompted by a second-opinion review, complementary to the `predicted (unwrapped)` vs. `true` scatter already in use for exactly the heads where that scatter can look "boxed." No styling beyond matplotlib defaults. | done → [`rose_plot_output/`](rose_plot_output/) — confirms the existing read, no surprises. See `NOTES.md`. |
| `python experiments/phic_psi_poc-redo/sky_position_mollweide.py` | Bare-minimum Mollweide plot, one thin line per sample from true to predicted sky position (500 samples/model) — checks whether the ~85–90° mean angular separation already reported is uniformly bad or hides a bimodal (some-good/some-antipodal) pattern. No styling beyond matplotlib defaults. | done → [`sky_mollweide_output/`](sky_mollweide_output/) — **inconclusive, too dense to read.** Superseded by the row below. |
| `python experiments/phic_psi_poc-redo/sky_angular_separation_hist.py` | Histogram + CDF of per-sample angular separation — the direct, legible version of the question `sky_position_mollweide.py` couldn't answer. Uses the canonical `gwml.data.sky_transform` helpers, not a reimplementation. | done → [`sky_mollweide_output/sky_angular_separation.{png,pdf}`](sky_mollweide_output/) — **all 4 models: uniform-on-sphere null shape, no bimodal structure.** Sky localization genuinely uninformative, not a hidden accurate/antipodal mix. |
| `python experiments/phic_psi_poc-redo/bootstrap_sky_separation.py` | Formal shuffle-null bootstrap significance test (10,000 iterations) on `sky_position`'s mean angular separation, all 4 models — same procedure as `bootstrap_ang_mae.py`, closes the descriptive-only caveat left by the histogram above. | done → [`bootstrap_output/`](bootstrap_output/) — **significant for all 4 models; `cnn_attention` clears the 0.10 rad materiality floor (Δ=12.58°, z=+23.23σ) — a genuinely new, unresolved finding.** See `NOTES.md`. |
| `python experiments/phic_psi_poc-redo/snr_stratification_sky_position.py` | SNR-tercile stratification for `sky_position` — the decisive discriminator for `cnn_attention`'s result: monotonic improvement with SNR + high-SNR tercile independently clearing the floor ⇒ real per-sample recovery; floor-clearing but flat/non-monotonic ⇒ population-bias signature. Mirrors `preregistration_lam_retune.md`'s Step 3 exactly. | done → [`snr_output/`](snr_output/) — **`cnn_attention`: REAL-RECOVERY-CONSISTENT** (monotonic across all 3 terciles, high-SNR Δ=+15.53° clears the floor). `poc_a`/`poc_b`/`tcn`: NULL, unchanged. See `NOTES.md`. |

Full synthesis of the first six: `NOTES.md`'s 2026-09-18 entry. The rose plot, Mollweide plot, and its angular-separation-histogram replacement are exploratory follow-ups from a second-opinion review — see `NOTES.md`'s "second-opinion review" entry and its rose-plot/Mollweide results entry for why they were added and what they found.

**Stage 2b(i): eleven of twelve scripts done.** Only `perturbation_trace_standalone.py final` remains, parked per its own calibration-failure rule. The `cnn_attention`/`sky_position` thread is closed — REAL-RECOVERY-CONSISTENT, the first result in this investigation to pass every check this repo's discipline requires of a positive finding.
