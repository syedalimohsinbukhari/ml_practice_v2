#!/usr/bin/env python
"""λ=0.10 retune diagnostics — mechanical verdict per preregistration_lam_retune.md.

Three checks against the freshly trained λ=0.10 checkpoints:
...
Run this after run_lam005_retune.py if λ=0.05 doesn't clear the Step 0 gate.

Usage (on GPU machine, after training):
    python experiments/phic_psi_poc/diagnostic_lam010_retune.py
"""
...
def _setup_logging(out_dir):
    ...
    log_path = out_dir / f"diagnostic_lam010_retune_{ts}.log"
...
CONFIGS = {
    "poc_a (baseline)": EXPERIMENTS_DIR / "config_lam010_retune.yaml",
    "tcn": EXPERIMENTS_DIR / "config_lam010_retune_tcn.yaml",
}
...
def main():
    ...
    for label, config_path in CONFIGS.items():
        ...
        if not gate["passed"]:
            print(f"\n  → UNINTERPRETABLE at λ={LAMBDA_LABEL}. λ alone may be "
                  f"insufficient for this architecture/head — consider "
                  f"architecture-level fixes rather than a further λ sweep.")
    ...
    # Markdown report
    # ==================================================================
    md_path = OUT_DIR / f"diagnostic_lam010_retune_{ts}.md"
