#!/usr/bin/env python
"""Bootstrap CI on ang_MAE for periodic heads across the redo's certified 4-model set.

Redo counterpart of ``experiments/phic_psi_poc/bootstrap_ang_mae.py``, copied verbatim
in logic and repointed at this directory's own configs/checkpoints — same "redo every
check the original ran" rationale as inclination_stratification.py's header. Broader
in scope than, and not gated by, ``preregistration_lam_retune.md``'s Steps 1-3: this
tests all 3 periodic heads (coa_phase, polarization_angle, inclination) across all 4
down-select-confirmed models directly, independent of whether poc_redo_b's Step 0
std_ratio gate ever clears — Steps 1-3 test only poc_redo_b's combo_A/combo_B, and
only if that gate passes (it hasn't, at any λ tried).

Particularly relevant given this session's finding (NOTES.md, 2026-09-16):
`cnn_attention`'s training-split circular loss dropped to ~0.55 (memorization) while
its validation loss stayed flat at ~1.0. This script predicts on the validation split,
so it is the direct test of whether that flat validation number is genuinely
indistinguishable from chance, or hides a small real signal the aggregate loss masks.
...
per this repo's CLAUDE.md CPU-only rule):
    python experiments/phic_psi_poc-redo/bootstrap_ang_mae.py
...
def main():
    ...
    print("=" * 100)
    print("BOOTSTRAP CI ON ang_MAE — periodic heads, redo certified 4-model set (REDO)")
    ...
    with open(md_path, "w") as f:
        f.write(f"# Bootstrap CI on ang_MAE — Periodic Heads (Redo)\n\n")
