...
Redo counterpart of ``experiments/phic_psi_poc/inclination_stratification.py``, copied
verbatim in logic and repointed at this directory's own configs/checkpoints — see
``experiment_index.md``'s "carried over unchanged" convention. Not run because the redo's
own combo result came out differently (it didn't — as of 2026-09-16 it's still flat/
UNINTERPRETABLE, same as the original at every λ tried); run because the redo's mandate
is to redo every check the original performed under the corrected `2φc±2ψ` formula, not
just the ones expected to change. A null result here, on the redo's own checkpoints, is
still evidence the original's finding wasn't an artifact of testing the wrong formula —
that only follows if this check is actually rerun, not assumed to transfer.

Companion to snr_stratification.py (Section E of the original's Run 7 verification plan),
on the one axis that was never separately reported: does ang_MAE for phi_c/psi improve
toward edge-on systems, where the analytic degeneracy of the thesis chapter's Section 3
is weakest? Bands mirror that section's face-on/edge-on split (|cos iota| > 0.9 and < 0.5).

CONFIGS below is this redo's own certified 4-model set (poc_a, poc_b, tcn, cnn_attention —
NOTES.md, 2026-09-15 down-select re-validation), not the original's. HEADS' periods/null
expectations are unchanged from the original: `coa_phase`/`polarization_angle`'s own
PERIODIC-head periods in `heads_spec.py` were never touched by the combo-formula fix
(redo_procedure.md §0 — the fix lives in how the PoC's combo construction *uses* these
vectors, not in the heads' own period declarations).

Usage (on GPU machine — loads best.weights.h5, calls trainer.predict; not run locally
per this repo's CLAUDE.md CPU-only rule):
    python experiments/phic_psi_poc-redo/inclination_stratification.py
...
def main():
    ...
    print("=" * 100)
    print("INCLINATION-STRATIFIED ang_MAE — final epoch, all models, all periodic heads (REDO)")
    ...
    with open(md_path, "w") as f:
        f.write(f"# Inclination-Stratified ang_MAE — All Periodic Heads (Redo)\n\n")
        f.write(f"**Generated**: {ts}\n")
        f.write(f"**Bands**: face-on |cos(iota)| > {FACE_ON_COS_IOTA}, "
                f"edge-on |cos(iota)| < {EDGE_ON_COS_IOTA}, mixed in between "
                f"(matches original thesis chapter Section 3)\n")
        ...
        f.write("The chapter's analytic prerequisite study finds the phi_c-psi degeneracy "
                ...
                "degeneracy is exact across the tested population, not just at face-on. This "
                "is the redo's own run against the corrected `2φc±2ψ` formula's checkpoints, "
                "not a reuse of the original's (wrong-formula) result.\n\n")
