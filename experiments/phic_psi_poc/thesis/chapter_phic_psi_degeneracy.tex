...
The fully certified evidential base --- the subset that also passed the pre-declared magnitude-health interpretability gate (\S\ref{sec:phicpsi:pool}) --- is two $\lambda$-matched configurations, poc\_b and cnn\_attention; the remaining five corroborate under progressively relaxed certification, not as independent repeats of the same test.
...
Marginal parameter distributions (verified empirically; \S\ref{sec:phicpsi:suite}, Check~1): chirp mass 8.84--43.45\,$M_\odot$, mass ratio 0.20--1.00, network SNR uniform in $[7, 15]$, and --- critically --- $\varphi_c$, $\psi$, $\iota$, and right ascension uniform over their supports ($\iota$ uniform in $\iota$ itself rather than in $\cos\iota$, a deviation from the astrophysically correct isotropic prior discussed in \S\ref{sec:phicpsi:threats}), with declination following the cos-weighted sky prior.
...
\begin{table}[tbp]
    \centering
    \caption{Inclination-stratified ang\_MAE, edge-on band vs null (rad).}
...
\item \textbf{Waveform-model provenance is asserted, not recorded.}
All injections are understood to use the dominant quadrupole ($\ell=2$, $|m|=2$) mode only (IMRPhenomD, no higher-order modes) --- the condition under which the face-on degeneracy argument of \S\ref{sec:phicpsi:intro} is exact.
...
If this recollection were wrong and higher-order modes were in fact present, the degeneracy would be only approximately exact even face-on, which would \emph{sharpen} rather than weaken the null (a network failing to exploit even a HOM-broken degeneracy is stronger evidence of no recoverable signal) --- but the provenance gap itself should be closed by regenerating the dataset with a script that records its own configuration.
...
(The inclination-stratified breakdown originally listed here as an immediate check has since run; its results are in \S\ref{sec:phicpsi:inclination} and no longer future work.)
