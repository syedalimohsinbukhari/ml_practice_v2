% On the Recoverability of Coalescence Phase and Polarization Angle
% from Gravitational-Wave Strain: A Systematically Verified Null Result
%
% Standalone LaTeX source for arXiv submission. Self-contained: no external
% class or preamble is required beyond the packages loaded below, all of
% which are present in arXiv's TeX Live distribution.
%
% Adapted from the thesis chapter of the same title
% (experiments/phic_psi_poc/thesis/chapter_phic_psi_degeneracy.tex) on
% branch poc/phic-psi-degeneracy. Content and structure are carried over
% essentially unchanged (a "light trim"): only thesis-internal scaffolding
% (draft-status banner, adversarial-review bookkeeping, cross-references to
% "the wider thesis") has been removed, and paper front matter (title,
% author, abstract, a resolved bibliography) has been added. All
% quantitative values are taken verbatim from the experiment record; the
% claim-to-artifact map in the Appendix traces every number to a versioned
% file in the accompanying repository.
%
% Figures are referenced from the experiment output directories one level
% up (see \graphicspath below). Before an actual arXiv upload, gather the
% referenced PNG files into the submission directory alongside this .tex
% (arXiv compiles from a flat/self-contained source tree) -- they are left
% in place here rather than duplicated into paper/.
...
\begin{abstract}
The coalescence phase $\varphi_c$ and polarization angle $\psi$ of a compact-binary gravitational-wave signal are, for the dominant quadrupole radiation mode, degenerate with each other in the face-on limit: only a single combination, effectively $\varphi_c \pm 2\psi$, is constrained by the strain, and the individual angles disentangle only as the system moves toward edge-on. We test directly whether a point-estimating deep neural network can recover $\varphi_c$ and $\psi$ from two-detector strain --- individually, or through their better-conditioned sum/difference combination --- across five trunk architectures, two head parameterizations, and four magnitude-regularization strengths, roughly nine full training campaigns in all. The answer is a defended null: every configuration converges to the random-guessing baseline on both angles, while the same networks recover chirp mass, merger time, signal-to-noise ratio, and sky position with high fidelity from the same shared representation. The null survives a verification battery --- configuration and code-path audits, a 10{,}000-permutation label-shuffle bootstrap, SNR- and inclination-stratified analysis, a regularization ablation, and a pre-registered two-arm sweep with mechanically evaluated success criteria --- and is corroborated by an analytic prerequisite study that independently bounds how little conditioning structure the combination carries ($\approx 1.16\times$ on average, vanishing face-on). Two of the seven configurations reach a fully certified interpretability status; the remainder corroborate the same result under progressively relaxed certification. This null does not reopen a settled question: neural-posterior-estimation methods already represent this degeneracy correctly by construction, since a full posterior need not commit to a single value for an unidentifiable target. What point estimation exposes instead is a practical hazard for the point-estimate and hybrid pipelines still built on regression heads of this kind: the failure is not obvious, and for a substantial stretch of this investigation it mimicked partial success. Beyond the physics result, we report the diagnostic discipline that made the null trustworthy: two genuine, initially invisible optimization pathologies --- a saturating output activation and a norm-blind loss that let raw output magnitude drift unboundedly --- had to be found and mechanistically understood before flat loss curves could be read as evidence about the underlying physics rather than as engineering failure, and every subsequent metric was validated at the mechanism level before being trusted.
\end{abstract}
...
Across the eight $\varphi_c/\psi$ tests the null-CI half-widths range ${\approx}0.002$--$0.023$\,rad, so this is not merely ``not significant'' --- it is a comparatively tight bound against even a small real effect for most model--head pairs.
...
Its effect size is $\Delta = 0.038$\,rad ($2.2^\circ$), and that effect is uniform across SNR terciles (1.526/1.548/1.527) --- flat, not increasing with signal strength.
...
Of the eight $\varphi_c/\psi$ deviations, the largest in the \emph{hoped-for} direction is poc\_a/$\varphi_c$ ($+0.0241$\,rad, and the only one of the eight monotonic across all three bands: $1.6042 \to 1.5623 \to 1.5467$); the largest in the \emph{anti} direction is tcn/$\varphi_c$ ($-0.0407$\,rad).
...
That floor is $0.0569$\,rad for poc\_a, $0.0215$ for poc\_b, $0.0225$ for tcn, and $0.0944$ for cnn\_attention (the last consistent with its already-diagnosed high-variance attention-pooling readout, \S\ref{sec:phicpsi:confounds}) --- and every $\varphi_c/\psi$ deviation above falls at or below its own model's floor, with one exception.
tcn/$\varphi_c$'s $-0.0407$\,rad edge-on deviation exceeds tcn's own $0.0225$-rad floor, but in the \emph{worse-than-null} direction and on the one head this paper already flags as never achieving certified metric health (std\_ratio never converged, \S\ref{sec:phicpsi:confounds}, \S\ref{sec:phicpsi:prereg}): an unstable head producing an oversized swing, in the direction a real signal could not produce, is consistent with its documented instability, not a new finding.
...
\begin{table}[tbp]
    ...
    \begin{tabular}{@{}lcccccc@{}}
        \toprule
        Model & face-on $R^2$ & mixed $R^2$ & edge-on $R^2$ & $R^2$ spread & MAE spread ($M_\odot$) \\
        \midrule
...
$R^2$ stays within $[0.92, 0.97]$ for every model, with band-to-band $R^2$ spread of only $0.005$--$0.010$ and MAE spread of $0.034$--$0.080\,M_\odot$ against a baseline MAE of $0.95$--$1.37\,M_\odot$: the banding scheme itself does not inject variance at a scale that could explain the $\varphi_c/\psi$ deviations in Table~\ref{tab:phicpsi:inclination}.
