...
Redo counterpart of ``experiments/phic_psi_poc/analyse_predictions.py``, copied
verbatim in logic and repointed at this directory's own configs/checkpoints — same
"redo every check the original ran" rationale as inclination_stratification.py's
header. Table 4's "quick health check" and its COLLAPSE grade (circ_r > 0.9 and
angular_mae > 0.5) are exactly the systematic version of the manual scatter-plot
check that caught this session's `lam010_retune_b`/coa_phase std_ratio false-positive
— running this against the redo's own checkpoints checks whether it would have
flagged the same collapse mechanically, on this and the other configs, without a
by-hand PNG read.

Plots here additionally save `.pdf` alongside `.png` (the original only wrote `.png`)
per this repo's current CLAUDE.md figure convention, already followed elsewhere in
this directory (e.g. `diagnostic_logvar_gate.py`) — not a change to the original's
logic, just bringing plot output in line with the convention now in force.
...
Usage (on GPU machine):
    python experiments/phic_psi_poc-redo/analyse_predictions.py
"""
...
CONFIGS = {
    ...
    # "cnn_baseline":        ROOT / "experiments/phic_psi_poc-redo/config_cnn_baseline.yaml",
    "cnn_attention":       ROOT / "experiments/phic_psi_poc-redo/config_cnn_attention.yaml",
    # "inception_time":      ROOT / "experiments/phic_psi_poc-redo/config_inception_time.yaml",
    # "resnet1d":            ROOT / "experiments/phic_psi_poc-redo/config_resnet1d.yaml",
}
...
def main():
    ...
    print("=" * 100)
    print("COMPREHENSIVE PREDICTION ANALYSIS — all heads, all models (REDO)")
    print("=" * 100)
    ...
    out_dir = Path("experiments/phic_psi_poc-redo/analysis_output")
    ...
    with open(md_path, "w") as f:
        f.write(f"# Prediction Analysis Report (Redo) — {ts}\n\n")
...
def _generate_plots(results, out_dir, ts):
    ...
    for h, period, title in [("coa_phase", 2*np.pi, "φc (coa_phase)"),
        ...
        pdf_path = out_dir / f"histogram_{h}_{ts}.pdf"
        fig.savefig(png_path, bbox_inches="tight")
        fig.savefig(pdf_path, bbox_inches="tight")
        plt.close(fig)
        print(f"Plot: {png_path} / {pdf_path}")
    ...
    for h, xlabel in [("mchirp", r"$\mathcal{M}_c$ true"),
        ...
        pdf_path = out_dir / f"scatter_{h}_{ts}.pdf"
        fig.savefig(png_path, bbox_inches="tight")
        fig.savefig(pdf_path, bbox_inches="tight")
        plt.close(fig)
        print(f"Plot: {png_path} / {pdf_path}")
...
def _plot_health_heatmap(results, out_dir, ts):
    ...
    pdf_path = out_dir / f"health_check_{ts}.pdf"
    fig.savefig(png_path, bbox_inches="tight")
    fig.savefig(pdf_path, bbox_inches="tight")
    plt.close(fig)
    print(f"Plot: {png_path} / {pdf_path}")
