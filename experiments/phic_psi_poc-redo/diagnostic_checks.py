...
Redo counterpart of ``experiments/phic_psi_poc/diagnostic_checks.py``, copied in
logic and repointed at this directory's own configs/checkpoints/run directories —
same "redo every check the original ran" rationale as inclination_stratification.py's
header. Two corrections beyond path repointing, both load-bearing, not cosmetic:

  - CHECK 6 (gradient chain) originally replicated ``_poc_total_loss`` using the
    WRONG pre-correction combo (``complex_mul(z_phic_norm, z_psi_norm)`` — no
    doubling). Ported here with the ``tf_double_angle(z_phic_norm)`` step this
    redo's own ``trainer.py``/``_build_combo_vectors`` actually uses (A.2) —
    porting it unfixed would have silently traced the wrong formula's gradient
    chain from inside the redo directory itself.

  - CHECKS 5 and 7 (tanh saturation, early-training tanh saturation timing) are
    NOT ported as run-time checks. They are structurally inapplicable, not just
    uninteresting: this redo uses ``activation="linear"`` for PERIODIC heads from
    the very first run (NOTES.md's design-decisions table — tanh saturation was
    the original's own Run 3 root cause, already fixed, not being rediscovered).
    A "|value| > 0.99" saturation heuristic is meaningless for an unbounded linear
    output — running it would manufacture a false signal, not skip a redundant
    one. Their function bodies are kept below for reference/history but are not
    called from ``main()``; if a future redo run needs to re-litigate the
    activation choice, they can be revived with the tanh-specific heuristic
    replaced first.

Four of the original's seven checks are ported and run, cheapest first:
  ...
  3. LOG_VAR TRAJECTORY — pull the log_var (uncertainty weight) values over
     training for coa_phase / pol_angle / combo_A / combo_B, across all 7
     Round-1 architectures (broader scope than `diagnostic_logvar_gate.py`,
     which is the pre-registered, authoritative version of this question for
     the certified 4-model set specifically — this check's role is the wider
     descriptive survey, not a replacement).  Look for runaway/collapse
     dynamics early in training.
  ...
  6. GRADIENT CHAIN — trace gradient norms through the corrected combo
     construction stage by stage (see correction note above).

Usage (on GPU machine):
    python experiments/phic_psi_poc-redo/diagnostic_checks.py
"""
...
def check_loss_function():
    ...
    for config_name, config_path in [
        ...
        ("tcn (TNC)", ROOT / "experiments/phic_psi_poc-redo/config_tcn.yaml"),
        ("cnn_attention (ATX)", ROOT / "experiments/phic_psi_poc-redo/config_cnn_attention.yaml")
    ]:
        print(f"\n--- {config_name} ---")
...
def _plot_logvar_trajectories(out_dir):
    ...
    runs = {
        "poc_a": ROOT / "runs/phic_psi_poc_redo_a",
        "poc_b": ROOT / "runs/phic_psi_poc_redo_b",
        "tcn": ROOT / "runs/phic_psi_redo_tcn",
        "cnn_baseline": ROOT / "runs/phic_psi_redo_cnn_baseline",
        "cnn_attention": ROOT / "runs/phic_psi_redo_cnn_attention",
        "inception_time": ROOT / "runs/phic_psi_redo_inception_time",
        "resnet1d": ROOT / "runs/phic_psi_redo_resnet1d",
    }
...
def check_gradient_routing():
    ...
    config_path = ROOT / "experiments/phic_psi_poc-redo/config_poc.yaml"
    cfg = load_config(str(config_path))
    run_dir = latest_run_dir(cfg)
...
def check_gradient_chain():
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        # ── Step 2a: double phi_c (A.1.5) — the corrected formula's one
        # departure from the original's gradient chain (2phi_c+/-2psi, not
        # phi_c+/-2psi). Watched separately so the chain can report the
        # doubling stage's own gradient norm alongside the others.
        z_2phic_norm = tf_double_angle(z_phic_norm)
        tape.watch(z_2phic_norm)

        # ── Step 2b: Complex multiply to build combo vectors ──
        combo_A_pred = tf_complex_mul(z_2phic_norm, z_psi_norm)
        combo_B_pred = tf_complex_mul_conj(z_2phic_norm, z_psi_norm)
        tape.watch(combo_A_pred)
        tape.watch(combo_B_pred)

        # True combo vectors (unit by construction; normalise for safety)
        z_2phic_true = tf_double_angle(tf_normalize_unit(targets["coa_phase"]))
    ...
    _report_gn("dL/d(z_2phic_norm) [after double_angle, A.1.5]", z_2phic_norm)
...
def main():
    ...
    print("PHIC_PSI DEEP DIAGNOSTICS (REDO)")
    print("=" * 80)
    ...
    # check_tanh_saturation() / check_early_training_saturation() intentionally
    # NOT called — structurally inapplicable, see module docstring: this redo
    # uses activation="linear" for PERIODIC heads from day 1, so there is no
    # tanh to saturate. Kept defined below for reference only.
    check_gradient_chain()

    print("\n\nAll checks complete (5 of 7 original checks — see module docstring "
          "for why 2 are intentionally omitted, not skipped for time).")
    _teardown_logging()
